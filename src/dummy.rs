...
macro_rules! dummy {
    ($struct: ident, $locale: ident ) => {
        match $locale {
            ...
            _       => panic!("Anon: $locale is not a supported locale for $struct")
        }
    }
}
...
//            "ar_SA" => $struct(AR_SA,crate::range_usize!($range_i32)).fake::<Vec<String>>().join(" "),
            ...
            _       => panic!("Anon: {} is not a supported locale",$locale),
        }
    }
}

#[macro_export]
macro_rules! dummy_fr_only {
    ($struct: ident, $locale: ident ) => {
        match $locale {
            "fr_FR" => $struct(FR_FR).fake(),
...
pub(crate) use dummy_fr_only;

///
/// Create 2 Rust functions for a given fake-rs Struct
///
/// For instance:
...
#[macro_export]
macro_rules! declare_fn_with_range_to_string {
    ($name: tt, $struct: ident) => {
        paste::paste! {
            #[pg_extern]
            pub fn [ < $name _locale > ](
...
pub(crate) use declare_fn_with_range_to_string;
